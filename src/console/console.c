/*
 * Copyright (c) 2008-2009 Travis Geiselbrecht, 2019 Elliot Berman
 *
...
static task_t console_task = NULL;
...
static cmd_block sentinel_command = {
  .next = NULL,
  .count = 0,
  .list = NULL,
};
...
static cmd_block *command_list = &sentinel_command;
...
int console_init(void) {
    command_lock = mutex_create();

    // printf("start: %p end: %p\n", &__commands_start, &__commands_end);
...
static int cmd_repeat(int argc, const cmd_args *argv) {
    ...
    for (int i = 0; i < times; ++i) {
        printf("[%d/%d]\n", i + 1, times);
        int result = console_run_script(line);
        if (result != 0) {
...
//      printf(SPEW, "c 0x%hhx state %d arg %d inpos %d pos %d\n", c, state, arg, inpos, outpos);
...
static void convert_args(int argc, cmd_args *argv) {
    ...
    for (i = 0; i < argc; i++) {
        unsigned long u = strtoul(argv[i].str, &ign, 10);
        argv[i].u = u;
        argv[i].p = (void *)u;
...
static int command_loop(int (*get_line)(const char **, void *), void *get_line_cookie, bool showprompt, bool locked) {
    bool exit;
#if WITH_LIB_ENV
    bool report_result;
#endif
    ...
    while (!exit) {
       ...
       //     printf("%d: '%s'\n", i, args[i].str);
        ...
        if (!locked)
            mutex_take(command_lock, TIMEOUT_MAX);
...
void console_start(void) {
    ...
    // printf(INFO, "entering main console loop\n");


    while (command_loop(&read_debug_line, NULL, true, false) == 0)
        ;

    // printf(INFO, "exiting main console loop\n");

    free (debug_buffer);
}

void console_start_task_fn(void* ign) {
  console_init();
  console_start();
}

void console_start_task(void) {
  console_task = task_create(console_start_task_fn, NULL, TASK_PRIORITY_DEFAULT,
                             TASK_STACK_DEPTH_DEFAULT, "console");
}
...
int console_run_script(const char *string) {
    return console_run_script_etc(string, task_get_current() == console_task);
}
